...
SELECT saleprice --don't need the field you are filtering by
FROM houseprices
WHERE yearbuilt != 1984;
...
SELECT saleprice, yearbuilt AS yb
FROM houseprices
WHERE yb > 1990;  --queries yb before it technically exists
...
SELECT TOP 2 saleprice, yearbuilt      --used in other languages.
FROM houseprices;
...
SELECT saleprice AS price,
		CASE
		 WHEN yearbuilt > 1992 THEN 'Younger than me'
		 WHEN yearbuilt > 1989 THEN 'Younger than my sister'
		 ELSE 'Older than us'
		END AS year_label
FROM houseprices
WHERE street = 'Pave' AND saleprice BETWEEN 200000 AND 300000
LIMIT 5;
